  ...
  <div id='update_list'>
    <div class="mainhead">
      <h1 class="main-heading">
        <span class="main-heading-primary"> Update List </span>
      </h1>
    </div>
    ...
    <div class="container">
      <div class='form'>
        <form action="">
          ...
          <div class='list_name'>
            <input type="text" placeholder="Name" style="width: 620px;" class="form__field" name='list_description' required v-model="formData.list_name" />
          </div>
          ...
          <div class='submit'>
            <button class="btn" style="color: ghostwhite;" @click.prevent="updateList" alt="Submit"> &#10004; </button>
          </div>
  ...
  methods: {
    updateList() {
      if (this.formData.list_name == null || this.formData.list_description == null) {
        ...
        this.putList(`http://localhost:8080/api/user/list`, data).then((response) => {
        if (response.ok) {
          alert('List updated!')
          ...
          console.log('Could not update the list!')
        }
      })
      }
    },

    async putList(url = '', data = {}) {
